...
from project import y_train, preprocess_image, hog_extraction  # Assuming these are your project-specific functions

def read_folders_from_path(path):
...
def validate_model(bagging_classifier, scaler, max_length, folder_classification_path):
    ...
    for class_number in read_folders_from_path(folder_classification_path):
        class_path = os.path.join(folder_classification_path, class_number, 'Validation')
...
def validate_model_test(bagging_classifier, scaler, max_length, test_path):
    ...
    for class_number in read_folders_from_path(test_path):
        class_path = os.path.join(test_path, class_number, 'Validation')
        if os.path.isdir(class_path):
            for image_file in os.listdir(class_path):
                image_path = os.path.join(class_path, image_file)
                preprocessed_image = preprocess_image(image_path)
